...
def lazy_splinecst(pose,Pcorr,#should get corrected probability as input
                   dcut=20.0,
                   reportf=None,debug=False):
    #dat = np.load(npz)['estogram']
    n = len(Pcorr)
    ...
    for i in range(n-4):
        ...
        for j in range(i+4,n):
            ...
            id1 = rosetta.core.id.AtomID(atm_i,i+1)
            id2 = rosetta.core.id.AtomID(atm_j,j+1)
            xyz_i = pose.xyz(id1)
            xyz_j = pose.xyz(id2)
            d0 = xyz_i.distance(xyz_j)
            if d0 > dcut or sum(Pcorr[i][j]) < 1.0e-3: continue

            xs = d0+DELTA
            ys = -np.log(Pcorr[i][j]+1.0e-3)
            ys -= ys[7] #subtract center

            xs_v = rosetta.utility.vector1_double()
            ys_v = rosetta.utility.vector1_double()
            ...
            func = rosetta.core.scoring.func.SplineFunc("", 1.0, 0.0, 1.0, xs_v, ys_v)
            cst = rosetta.core.scoring.constraints.AtomPairConstraint( id1, id2, func )
            pose.add_constraint( cst )
            if reportf != None:
                form = '%3d %3d %8.3f'+' %6.3f'*len(ys)+'\n'
                out.write(form%tuple([i+1,j+1,d0]+list(ys)))
    ...
    # just for testing...
    if debug:
        sfxn = create_score_function("empty")
