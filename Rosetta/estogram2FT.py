...
def add_default_options_if_missing(opt_in):
    opt = copy.deepcopy(opt_in)
...
def estogram2sub(estogram,SS3,ulrs,opt):
        ...
        for isub,sub in enumerate(outputs):
            ...
            if len(subdef) > 1:
                ...
                for i,SSchunk in enumerate(subdef):
                    print( "SSchunk %d: %d-%d"%(i, SSchunk[0], SSchunk[-1]) )
                    for res in SSchunk: covered[res] = True
            #print(isub,len(sub)," %d"*20%tuple(covered[110:130]))

        print( "Confcut, covered: %.2f %3d"%(confcut,covered.count(True)))
    ...
    if confcut > MINCONFCUT:
        print( "Convereged at confcut %.2f, coverage/threshold: %d/%d"%(confcut, covered.count(True), ncover_min))
    else:
        print( "Terminated at confcut %.2f, coverage/threshold: %d/%d"%(confcut, covered.count(True), ncover_min))
...
def main(npz,pose,opt):
    ...
    opt_local = add_default_options_if_missing(opt)

    # predict ULR
    ulr_cons, ulr_aggr, maxdev, Qres = estogram2ulr(estogram,opt_local)

    # predict sub
    print( "Exclude: ", ulr_cons )
    subdef = estogram2sub(estogram,SS3,ulr_cons,opt_local)

    if opt.ulrmode == 'binary':
        ulroutput = (ulr_cons,ulr_aggr)
    else:
        ulroutput = maxdev
    return subdef, ulroutput
